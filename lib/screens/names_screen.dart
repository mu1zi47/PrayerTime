    ...
    return Scaffold(
      ...
      body: AnimatedBuilder(
        ...
        builder: (context, _) {
          ...
          final favoriteSource = AllahNames.all
          ...
          return Column(
            children: [
              ...
              Expanded(
                child: Stack(
                  children: [
                    // Scoped to just this PageView, not the back button/tab
                    // row above or the search bar below — a touch starting
                    // in the left/right strip Android reserves for its own
                    // back gesture never reaches this PageView, so it can't
                    // be mistaken for a tab swipe. Bottom guards against
                    // swipe-up-to-minimize the same way, scoped to just the
                    // scrolling content and not the search bar below.
                    EdgeGestureGuard(
                      left: true,
                      right: true,
                      bottom: true,
                      child: PageView(
                        ...
                        children: [
                          RepaintBoundary(
                            child: _NamesList(
                              names: allFiltered,
                              navigationSource: AllahNames.all,
                              favorites: favorites,
                          ...
                          RepaintBoundary(
                            child: _NamesList(
                              names: favFiltered,
                              // Back/Forward inside a name pushed from here
                              // stays within favorites — not the full list,
                              // and not narrowed further by any search text.
                              navigationSource: favoriteSource,
                              favorites: favorites,
                              appState: widget.appState,
                              bottomPadding: padding.bottom + _searchBarReserve,
...
class _NamesList extends StatelessWidget {
  ...
  final List<AllahName> navigationSource;
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      onTap: () => Navigator.of(context).push(
        MaterialPageRoute(
          builder: (_) => AllahNameDetailScreen(
            name: name,
            names: navigationSource,
            appState: appState,
          ),
        ),
      ),
